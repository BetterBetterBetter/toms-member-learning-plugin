...
class MemberLibrary_Homepage_Curation {
    ...
    public function add_menu_page() {
        $this->page_hook = (string) add_submenu_page(
            ...
            __('Library Homepage', 'member-library'),
            __('Homepage', 'member-library'),
            'edit_pages',
            self::PAGE_SLUG,
            array($this, 'render')
        );
    ...
    public function enqueue_assets($hook) {
        ...
        wp_enqueue_style(
            ...
            MEMBER_LIBRARY_PLUGIN_URL . 'assets/features/library-content/library-homepage-curation.css',
            array('tsol-library-content-admin'),
            MEMBER_LIBRARY_PLUGIN_VERSION
        );
        wp_enqueue_script(
            'tsol-library-homepage-curation',
            MEMBER_LIBRARY_PLUGIN_URL . 'assets/features/library-content/library-homepage-curation.js',
    ...
    public static function rails() {
        return array(
            'featured' => array(
                'label' => __('Featured', 'member-library'),
                'description' => __('A deliberately promoted mix of Courses and Series.', 'member-library'),
            ),
            'courses' => array(
                'label' => __('Courses', 'member-library'),
                'description' => __('Published Courses that are not Masterclasses.', 'member-library'),
            ),
            'masterclasses' => array(
                'label' => __('Masterclasses', 'member-library'),
                'description' => __('Courses from the Masterclasses Collection.', 'member-library'),
            ),
            'series' => array(
                'label' => __('Series', 'member-library'),
                'description' => __('Ongoing or finite Series.', 'member-library'),
            ),
        );
    }
    ...
    public function save() {
        if (!current_user_can('edit_pages')) {
            wp_die(esc_html__('You are not allowed to curate the Library homepage.', 'member-library'));
        }
        ...
        foreach ($affected_ids as $post_id) {
            MemberLibrary_Content_Changes::record_current_state((int) $post_id);
        }
    ...
    public function render() {
        ...
        <div class="wrap tsol-library-admin-page tsol-library-homepage" data-homepage-curation>
            ...
            <?php if (isset($_GET['updated'])) : ?>
                <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Homepage curation saved.', 'member-library'); ?></p></div>
            <?php endif; ?>

            <?php if (isset($_GET['conflict'])) : ?>
                <div class="notice notice-error"><p><?php esc_html_e('The Homepage changed in another browser tab. Your save was not applied; review the current order and try again.', 'member-library'); ?></p></div>
            <?php endif; ?>

            <div class="notice notice-info inline">
                <p><?php esc_html_e('Only published records are visible to ordinary visitors. Drafts can be arranged now and will remain hidden until they are published. Homepage placement never grants access and never changes search eligibility.', 'member-library'); ?></p>
            </div>
            ...
            <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post" data-homepage-form>
                ...
                <div class="tsol-library-homepage__toolbar">
                    <label for="tsol-library-homepage-search"><?php esc_html_e('Find content', 'member-library'); ?></label>
                    <input id="tsol-library-homepage-search" type="search" class="regular-text" placeholder="<?php esc_attr_e('Search Courses and Series', 'member-library'); ?>" data-homepage-search />
                    <span class="description"><?php esc_html_e('Search only filters this editing screen.', 'member-library'); ?></span>
                </div>
                        ...
                        <section class="tsol-library-homepage__rail" aria-labelledby="tsol-homepage-rail-<?php echo esc_attr($rail); ?>">
                            ...
                            <div class="tsol-library-homepage__list" data-homepage-list data-rail="<?php echo esc_attr($rail); ?>">
                                <?php foreach ($layout['rails'][$rail] as $post_id) : ?>
                                    ...
                                    <?php endif; ?>
                                <?php endforeach; ?>
                                <p class="tsol-library-homepage__empty" data-homepage-empty><?php esc_html_e('No content assigned.', 'member-library'); ?></p>
                            </div>
                        </section>
                    <?php endforeach; ?>
                </div>
                ...
                <section class="tsol-library-homepage__available" aria-labelledby="tsol-homepage-available-heading">
                    <header>
                        <h2 id="tsol-homepage-available-heading"><?php esc_html_e('Not on the homepage', 'member-library'); ?></h2>
                        <p><?php esc_html_e('These records remain available through their public URLs and search when published.', 'member-library'); ?></p>
                    </header>
                    <div class="tsol-library-homepage__list" data-homepage-list data-rail="">
                        ...
                        <?php endforeach; ?>
                        <p class="tsol-library-homepage__empty" data-homepage-empty><?php esc_html_e('Every Course and Series is assigned to a homepage section.', 'member-library'); ?></p>
                    </div>
                </section>

                <div class="tsol-library-homepage__actions">
                    <?php submit_button(__('Save homepage', 'member-library'), 'primary', 'submit', false); ?>
    ...
    private function render_record(WP_Post $post, $rail) {
        ...
        $kind = MemberLibrary_Content_Model::SERIES_POST_TYPE === $post->post_type
            ? __('Series', 'member-library')
            : (has_term('masterclasses', MemberLibrary_Content_Model::COURSE_COLLECTION_TAXONOMY, $post_id)
                ? __('Masterclass', 'member-library')
        ...
        <article class="tsol-library-homepage-record" data-homepage-record data-title="<?php echo esc_attr(strtolower((string) $post->post_title)); ?>" data-post-id="<?php echo esc_attr((string) $post_id); ?>">
            <span class="tsol-library-homepage-record__handle" data-homepage-handle title="<?php esc_attr_e('Drag to reorder', 'member-library'); ?>" aria-hidden="true">
                <span class="dashicons dashicons-menu" aria-hidden="true"></span>
            </span>
            <div class="tsol-library-homepage-record__identity">
                <strong><?php echo esc_html($post->post_title ?: __('(no title)', 'member-library')); ?></strong>
                <span><?php echo esc_html($kind); ?> · <?php echo esc_html($status_label); ?> <span data-homepage-position></span></span>
            </div>
            <label class="screen-reader-text" for="tsol-homepage-record-rail-<?php echo esc_attr((string) $post_id); ?>"><?php esc_html_e('Homepage section', 'member-library'); ?></label>
            <select id="tsol-homepage-record-rail-<?php echo esc_attr((string) $post_id); ?>" data-homepage-rail-select>
                <option value="" <?php selected('', $rail); ?>><?php esc_html_e('Not on homepage', 'member-library'); ?></option>
            ...
            <div class="tsol-library-homepage-record__actions">
                <button type="button" class="button button-small" data-homepage-move="up"><?php esc_html_e('Up', 'member-library'); ?></button>
                <button type="button" class="button button-small" data-homepage-move="down"><?php esc_html_e('Down', 'member-library'); ?></button>
                <?php if ($edit_url) : ?><a class="button button-small" href="<?php echo esc_url($edit_url); ?>"><?php esc_html_e('Edit', 'member-library'); ?></a><?php endif; ?>
            </div>
    ...
    private static function default_layout() {
        ...
        foreach ($records as $record) {
            if ((bool) get_post_meta($record->ID, MemberLibrary_Content_Model::META_FEATURED, true)) {
                $rails['featured'][] = (int) $record->ID;
            } elseif (MemberLibrary_Content_Model::SERIES_POST_TYPE === $record->post_type) {
                $rails['series'][] = (int) $record->ID;
            } elseif (has_term('masterclasses', MemberLibrary_Content_Model::COURSE_COLLECTION_TAXONOMY, $record->ID)) {
    ...
    private static function allowed_rails_for_post(WP_Post $post) {
        ...
        if (has_term('masterclasses', MemberLibrary_Content_Model::COURSE_COLLECTION_TAXONOMY, $post->ID)) {
