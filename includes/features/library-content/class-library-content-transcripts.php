...
class MemberLibrary_Content_Transcripts {
    ...
    public static function register_hooks() {
        ...
        add_action('save_post_' . MemberLibrary_Content_Model::ITEM_POST_TYPE, array(__CLASS__, 'save_upload'), 40, 3);
    ...
    public static function render_media_fields($post) {
        ...
        $hash = sanitize_text_field((string) get_post_meta($post->ID, MemberLibrary_Content_Model::META_TRANSCRIPT_HASH, true));
        $filename = sanitize_file_name((string) get_post_meta($post->ID, MemberLibrary_Content_Model::META_TRANSCRIPT_FILENAME, true));
        $modified_at = sanitize_text_field((string) get_post_meta($post->ID, MemberLibrary_Content_Model::META_TRANSCRIPT_MODIFIED_AT, true));
        ...
        <section class="tsol-library-transcript-editor" data-library-transcript-editor aria-labelledby="tsol-library-transcript-heading">
            <div class="tsol-library-section-intro">
                <div>
                    <h3 id="tsol-library-transcript-heading"><?php esc_html_e('Primary video transcript', 'member-library'); ?></h3>
                    <p><?php esc_html_e('Upload the transcript for the primary playback source in the first media row.', 'member-library'); ?></p>
                    <p class="description"><?php esc_html_e('Choose a UTF-8 WebVTT file up to 5 MB. Saving without choosing a file keeps the current transcript.', 'member-library'); ?></p>
                </div>
            </div>
            <div class="tsol-library-field tsol-library-field--wide">
                <label for="tsol-library-transcript-file"><?php esc_html_e('WebVTT transcript (.vtt)', 'member-library'); ?></label>
            ...
            <?php if ('' !== $hash) : ?>
                <div class="tsol-library-transcript-status" role="status">
                    <p>
                        <strong><?php esc_html_e('Current transcript:', 'member-library'); ?></strong>
                        <?php echo esc_html('' !== $filename ? $filename : __('WebVTT file', 'member-library')); ?>
                        <?php if ('' !== $modified_at) : ?>
                            <span class="description"><?php echo esc_html(sprintf(__('updated %s UTC', 'member-library'), $modified_at)); ?></span>
                        <?php endif; ?>
                    </p>
    ...
    public static function save_upload($post_id, $post, $update) {
        ...
        if (UPLOAD_ERR_OK !== $error || $size <= 0 || $size > self::MAX_FILE_BYTES || 'vtt' !== strtolower((string) pathinfo($filename, PATHINFO_EXTENSION))) {
            self::store_error($post_id, __('Choose a valid .vtt transcript no larger than 5 MB.', 'member-library'));
            return;
        }
        if (!is_uploaded_file($temporary_name) || !is_readable($temporary_name)) {
            self::store_error($post_id, __('The transcript upload could not be read. Please choose the file again.', 'member-library'));
            return;
        }
        $vtt = file_get_contents($temporary_name);
        $vtt = MemberLibrary_Content_Model::sanitize_transcript_vtt($vtt);
        if ('' === $vtt) {
            self::store_error($post_id, __('The selected file is not valid WebVTT.', 'member-library'));
            return;
        }

        $version = max(0, (int) get_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_VERSION, true)) + 1;
        $modified_at = gmdate('Y-m-d\TH:i:s.000\Z');
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_HASH, hash('sha256', $vtt));
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_LANGUAGE, 'en');
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_FILENAME, $filename);
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_MODIFIED_AT, $modified_at);
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_VERSION, $version);
        update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_CONTENT, wp_slash($vtt));
    ...
    public static function schedule_after_import($meta_id, $post_id, $meta_key, $meta_value) {
        ...
        if (MemberLibrary_Content_Model::META_TRANSCRIPT_CONTENT !== (string) $meta_key) {
            return;
        }
    ...
    public static function deliver($post_id) {
        ...
        $response = wp_remote_post($endpoint_url, array(
            ...
            'redirection' => 0,
            'blocking' => true,
            'user-agent' => 'TSOL-Library-Transcript/' . MEMBER_LIBRARY_PLUGIN_VERSION,
        ));
    ...
    public static function delivery_payload($post_id, $issued_at = null) {
        ...
        $vtt = (string) get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_CONTENT, true);
        $hash = strtolower((string) get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_HASH, true));
        $content_uuid = strtolower((string) get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_UUID, true));
        $media = get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_MEDIA_ASSETS, true);
        ...
        return array(
            ...
            'name' => 'English',
            'kind' => 'subtitles',
            'source_modified_at' => (string) get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_MODIFIED_AT, true),
            'source_version' => max(1, (int) get_post_meta((int) $post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_VERSION, true)),
            'content_sha256' => $hash,
            'vtt' => $vtt,
        );
    }
    ...
    private static function secret() {
        $secret = trim((string) apply_filters('tsol_library_catalogue_webhook_secret', MemberLibrary_Auth_Settings::catalogue_webhook_secret()));
    ...
    private static function store_error($post_id, $message) {
        $key = MemberLibrary_Content_Admin::NOTICE_PREFIX . get_current_user_id() . '_' . (int) $post_id;
