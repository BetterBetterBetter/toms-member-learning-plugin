...
class TSOL_Library_Resource_Normalizer {
    ...
    public static function from_url($url, $position = 1, $label = '') {
        ...
        if ('' === $url || (!$is_web && !$is_email)) {
            return new WP_Error('invalid_resource_url', __('Resource URL must be a web or email link.', 'libertyclassroom-library'));
        }

        if ($is_web && class_exists('TSOL_Library_Media_Normalizer')) {
            $media = TSOL_Library_Media_Normalizer::from_url($url);
            if (!is_wp_error($media)) {
                return new WP_Error('playable_media_url', __('Playable media is stored separately from lesson resources.', 'libertyclassroom-library'));
            }
        }
    ...
    private static function resource_label($label, $url, $path, $position) {
        ...
        return '' !== $host ? $host : sprintf(__('Resource %d', 'libertyclassroom-library'), $position);
    }
