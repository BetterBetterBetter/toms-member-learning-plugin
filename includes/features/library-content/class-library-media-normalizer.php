...
class TSOL_Library_Media_Normalizer {
    ...
    public static function from_url($url) {
        ...
        if ('' === $url) {
            return new WP_Error('empty_media_url', __('Enter a media URL.', 'libertyclassroom-library'));
        }

        $parts = wp_parse_url($url);
        if (!is_array($parts) || empty($parts['host'])) {
            return new WP_Error('invalid_media_url', __('Enter a valid absolute media URL.', 'libertyclassroom-library'));
        }
        ...
        if ($attachment_id > 0) {
            ...
            if (0 !== strpos($mime_type, 'video/') && 0 !== strpos($mime_type, 'audio/')) {
                return new WP_Error('unsupported_media_attachment', __('Choose a WordPress audio or video attachment. Documents belong in Library resources.', 'libertyclassroom-library'));
            }
            return array(
        ...
        if (!self::is_direct_media_url($url)) {
            return new WP_Error('unsupported_media_url', __('Use a Vimeo, YouTube, or direct audio/video URL.', 'libertyclassroom-library'));
        }
    ...
    public static function normalize_asset($asset, $position = 1) {
        if (!is_array($asset)) {
            return new WP_Error('invalid_media_asset', __('Media assets must be arrays.', 'libertyclassroom-library'));
        }
        ...
        if (!empty($asset['source_url'])) {
            ...
            if (!in_array($provider, array('vimeo', 'youtube', 'wordpress', 'external'), true)) {
                return new WP_Error('invalid_media_provider', __('Choose a supported media provider.', 'libertyclassroom-library'));
            }
    ...
    private static function vimeo_asset($url, $parts, $segments) {
        ...
        if ('' === $video_id) {
            return new WP_Error('invalid_vimeo_url', __('The Vimeo URL does not contain a video ID.', 'libertyclassroom-library'));
        }
    ...
    private static function youtube_asset($url, $parts, $segments, $host) {
        ...
        if ('' === $video_id) {
            return new WP_Error('invalid_youtube_url', __('The YouTube URL does not contain a video ID.', 'libertyclassroom-library'));
        }
