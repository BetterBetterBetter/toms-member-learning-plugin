...
class TSOL_Library_Announcement_Admin {
    ...
    public function title_placeholder($placeholder, $post) {
        return $post instanceof WP_Post && TSOL_Library_Announcement_Model::POST_TYPE === $post->post_type
            ? __('Announcement subject', 'tomschooloflife-plugin')
            : $placeholder;
    }
    ...
    public function add_meta_boxes($post_type, $post) {
        ...
        add_meta_box('tsol-announcement-message', __('1. Message', 'tomschooloflife-plugin'), array($this, 'render_message'), $post_type, 'normal', 'high');
        add_meta_box('tsol-announcement-destination', __('2. Destination', 'tomschooloflife-plugin'), array($this, 'render_destination'), $post_type, 'normal', 'high');
        add_meta_box('tsol-announcement-audience', __('3. Audience', 'tomschooloflife-plugin'), array($this, 'render_audience'), $post_type, 'normal', 'high');
        add_meta_box('tsol-announcement-save', __('Save announcement', 'tomschooloflife-plugin'), array($this, 'render_save'), $post_type, 'side', 'high');
        add_meta_box('tsol-announcement-delivery', __('4. Delivery', 'tomschooloflife-plugin'), array($this, 'render_delivery'), $post_type, 'side', 'default');
        add_meta_box('tsol-announcement-review', __('5. Review', 'tomschooloflife-plugin'), array($this, 'render_review'), $post_type, 'side', 'default');
        if (current_user_can(TSOL_Library_Announcement_Model::CAP_VIEW_DELIVERY)) {
            add_meta_box('tsol-announcement-audit', __('Editorial audit', 'tomschooloflife-plugin'), array($this, 'render_audit'), $post_type, 'normal', 'low');
        }
    }
    ...
    public function enqueue_assets() {
        ...
        wp_localize_script('tsol-library-announcement-admin', 'tsolAnnouncementAdmin', array(
            ...
            'strings' => array(
                'searching' => __('Searching…', 'tomschooloflife-plugin'),
                'noUsers' => __('No matching users found.', 'tomschooloflife-plugin'),
                'searchFailed' => __('User search is temporarily unavailable.', 'tomschooloflife-plugin'),
                'previewing' => __('Calculating the audience…', 'tomschooloflife-plugin'),
                'previewFailed' => __('The complete audience preview is temporarily unavailable.', 'tomschooloflife-plugin'),
                'saveBeforePreview' => __('Save the draft before previewing this changed audience.', 'tomschooloflife-plugin'),
                'removeUser' => __('Remove', 'tomschooloflife-plugin'),
                'membershipRequired' => __('Select at least one active membership.', 'tomschooloflife-plugin'),
                'specificUserRequired' => __('Select at least one WordPress user.', 'tomschooloflife-plugin'),
                'userLimit' => __('A maximum of 100 unique users can be selected across recipients and exclusions.', 'tomschooloflife-plugin'),
            ),
        ));
    }
    ...
    public function render_message($post) {
        ...
        <div class="tsol-announcement-fields">
            <label class="tsol-announcement-field" for="tsol-announcement-summary">
                <span class="tsol-announcement-field__label"><?php esc_html_e('Summary', 'tomschooloflife-plugin'); ?></span>
                ...
                <span class="description"><?php esc_html_e('A concise plain-text preview shown in the notification list. Maximum 500 characters.', 'tomschooloflife-plugin'); ?></span>
            </label>
            <div class="tsol-announcement-field">
                <span class="tsol-announcement-field__label"><?php esc_html_e('Optional detail', 'tomschooloflife-plugin'); ?></span>
                <p class="description"><?php esc_html_e('Use simple headings, lists, emphasis, and quotes. Links, media, embeds, scripts, forms, and tracking markup are removed.', 'tomschooloflife-plugin'); ?></p>
                ...
                <p class="description"><span data-announcement-body-count>0</span> / 5,000 <?php esc_html_e('characters', 'tomschooloflife-plugin'); ?></p>
            </div>
        </div>
        <?php
    }
    ...
    public function render_destination($post) {
        ...
        ?>
        <p><?php esc_html_e('Choose where the notification opens. A protected destination automatically locks its live MemberPress access check into the audience.', 'tomschooloflife-plugin'); ?></p>
        <?php if ($can_manage) : ?>
            <label class="tsol-announcement-field" for="tsol-announcement-destination-select">
                <span class="tsol-announcement-field__label"><?php esc_html_e('Destination', 'tomschooloflife-plugin'); ?></span>
                <select id="tsol-announcement-destination-select" name="<?php echo esc_attr(self::PAYLOAD_NAME); ?>[destination_id]">
                    <option value="0" <?php selected(0, $selected); ?>><?php esc_html_e('General announcement — Notifications page', 'tomschooloflife-plugin'); ?></option>
                    <?php if ($selected_unavailable) : ?>
                        <option value="<?php echo esc_attr($selected); ?>" selected data-unavailable-destination><?php esc_html_e('Unavailable destination — choose a replacement', 'tomschooloflife-plugin'); ?></option>
                    <?php endif; ?>
            ...
            <?php if ($selected_unavailable) : ?>
                <div class="notice notice-error inline"><p><?php esc_html_e('The saved destination is no longer published or has lost its School identity. Choose a replacement before previewing this audience.', 'tomschooloflife-plugin'); ?></p></div>
            <?php endif; ?>
        <?php else : ?>
            <p><strong><?php echo esc_html($this->destination_label($selected)); ?></strong></p>
            <p class="description"><?php esc_html_e('An administrator will review the destination and audience before any future delivery.', 'tomschooloflife-plugin'); ?></p>
        <?php endif; ?>
        <div class="notice notice-info inline"><p><?php esc_html_e('Selecting a destination never grants access. WordPress and MemberPress remain the live authority.', 'tomschooloflife-plugin'); ?></p></div>
        <?php
    }
    ...
    public function render_audience($post) {
        ...
        if (!$can_manage) {
            ...
            ?>
            <div class="notice notice-info inline"><p><strong><?php esc_html_e('Audience review is administrator-only.', 'tomschooloflife-plugin'); ?></strong></p></div>
            <p><?php echo esc_html($summary !== '' ? $summary : __('Everyone signed in to the School (administrator review required)', 'tomschooloflife-plugin')); ?></p>
            <?php
            return;
        }
        ...
        <fieldset class="tsol-announcement-presets" data-announcement-presets>
            <legend class="screen-reader-text"><?php esc_html_e('Audience preset', 'tomschooloflife-plugin'); ?></legend>
        ...
        <p class="description" data-announcement-destination-guidance <?php if ($has_destination) : ?>hidden<?php endif; ?>><?php esc_html_e('Choose a published Course or Series destination to use content-access or enrollment/following audiences.', 'tomschooloflife-plugin'); ?></p>
        ...
        <div class="tsol-announcement-conditional" data-audience-fields="active_membership" <?php if (TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP !== $preset) : ?>hidden<?php endif; ?>>
            <h3><?php esc_html_e('Selected memberships', 'tomschooloflife-plugin'); ?></h3>
            <p class="description"><?php esc_html_e('This chooses a message audience; it does not edit the membership or its MemberPress rules.', 'tomschooloflife-plugin'); ?></p>
            <p class="description"><strong data-membership-count><?php echo esc_html(count($membership_ids)); ?></strong> <?php esc_html_e('/ 20 selected', 'tomschooloflife-plugin'); ?></p>
        ...
        <div class="tsol-announcement-conditional" data-audience-fields="specific_users" <?php if (TSOL_Library_Announcement_Audience_Builder::PRESET_SPECIFIC_USERS !== $preset) : ?>hidden<?php endif; ?>>
            <?php $this->render_user_picker('specific_user_ids', __('Selected users', 'tomschooloflife-plugin'), $specific_ids); ?>
        </div>

        <details class="tsol-announcement-exclusions">
            <summary><?php esc_html_e('Exclude specific users', 'tomschooloflife-plugin'); ?></summary>
            <?php $this->render_user_picker('exclude_user_ids', __('Excluded users', 'tomschooloflife-plugin'), $exclude_ids); ?>
        </details>
        <div class="notice notice-warning inline"><p><?php esc_html_e('Audience choices select recipients only. Every protected destination is checked again against live MemberPress access.', 'tomschooloflife-plugin'); ?></p></div>
        <?php
    }
    ...
    public function render_delivery($post) {
        ...
        ?>
        <p><label><input type="checkbox" checked disabled> <strong><?php esc_html_e('In-app notification', 'tomschooloflife-plugin'); ?></strong></label></p>
        <p class="description"><?php esc_html_e('Email is outside the approved scope.', 'tomschooloflife-plugin'); ?></p>
        <?php if (current_user_can(TSOL_Library_Announcement_Model::CAP_MANAGE_AUDIENCE)) : ?>
            <label class="tsol-announcement-field" for="tsol-announcement-expiry">
                <span class="tsol-announcement-field__label"><?php esc_html_e('Optional visibility expiry', 'tomschooloflife-plugin'); ?></span>
                ...
                <span class="description"><?php echo esc_html(sprintf(__('Displayed in %s. Expiry never changes content access.', 'tomschooloflife-plugin'), wp_timezone_string())); ?></span>
            </label>
        <?php else : ?>
            <p><strong><?php esc_html_e('Visibility expiry', 'tomschooloflife-plugin'); ?>:</strong> <?php echo esc_html($local_expiry !== '' ? $local_expiry : __('None', 'tomschooloflife-plugin')); ?></p>
            <p class="description"><?php esc_html_e('An administrator will configure visibility expiry.', 'tomschooloflife-plugin'); ?></p>
        <?php endif; ?>
        <div class="notice notice-warning inline"><p><strong><?php esc_html_e('Publishing and scheduling are disabled.', 'tomschooloflife-plugin'); ?></strong> <?php esc_html_e('This phase saves drafts only.', 'tomschooloflife-plugin'); ?></p></div>
        <?php
    }
    ...
    public function render_save($post) {
        ...
        <p><button type="submit" name="save" class="button button-primary button-large"><?php esc_html_e('Save draft', 'tomschooloflife-plugin'); ?></button></p>
        <p class="description"><?php esc_html_e('Saving cannot publish, schedule, fan out, email, or change access.', 'tomschooloflife-plugin'); ?></p>
        <?php if ('auto-draft' !== $post->post_status && current_user_can('delete_post', $post->ID)) : ?>
            <p><a class="submitdelete deletion" href="<?php echo esc_url(get_delete_post_link($post->ID)); ?>"><?php esc_html_e('Move to Trash', 'tomschooloflife-plugin'); ?></a></p>
        <?php endif;
    }
    ...
    public function render_review($post) {
        ...
        <dl class="tsol-announcement-review-list">
            <div><dt><?php esc_html_e('Destination', 'tomschooloflife-plugin'); ?></dt><dd><?php echo esc_html($this->destination_label((int) get_post_meta($post->ID, TSOL_Library_Announcement_Model::META_DESTINATION_ID, true))); ?></dd></div>
            <div><dt><?php esc_html_e('Audience', 'tomschooloflife-plugin'); ?></dt><dd><?php echo esc_html($summary !== '' ? $summary : __('Save the draft to validate its audience.', 'tomschooloflife-plugin')); ?></dd></div>
            <div><dt><?php esc_html_e('Definition', 'tomschooloflife-plugin'); ?></dt><dd><code><?php echo esc_html($hash !== '' ? substr($hash, 0, 12) . '…' : '—'); ?></code></dd></div>
        </dl>
        <div data-announcement-preview-result aria-live="polite">
            ...
            <?php elseif (!$can_view_delivery) : ?>
                <p class="description"><?php esc_html_e('An administrator will complete the aggregate audience review.', 'tomschooloflife-plugin'); ?></p>
            <?php else : ?>
                <p class="description"><?php esc_html_e('No current complete preview.', 'tomschooloflife-plugin'); ?></p>
            <?php endif; ?>
        </div>
        <?php if (current_user_can(TSOL_Library_Announcement_Model::CAP_MANAGE_AUDIENCE)) : ?>
            <p><button type="button" class="button" data-announcement-preview data-post-id="<?php echo esc_attr($post->ID); ?>" <?php disabled(!TSOL_Library_Announcement_Flags::preview_enabled() || $hash === '' || 'auto-draft' === $post->post_status || !$destination_current); ?>><?php esc_html_e('Preview audience', 'tomschooloflife-plugin'); ?></button></p>
            <p class="description" data-announcement-preview-guidance><?php esc_html_e('Save any destination or audience changes before previewing.', 'tomschooloflife-plugin'); ?></p>
        <?php endif; ?>
        <p><button type="button" class="button" disabled><?php esc_html_e('Send test to me', 'tomschooloflife-plugin'); ?></button></p>
        <p class="description"><?php esc_html_e('Self-test delivery becomes available only after private School notification persistence is reviewed and enabled.', 'tomschooloflife-plugin'); ?></p>
        <?php
    }
    ...
    public function render_audit($post) {
        ...
        if (empty($entries)) {
            echo '<p class="description">' . esc_html__('No editorial audit events have been recorded.', 'tomschooloflife-plugin') . '</p>';
            return;
        }
        echo '<table class="widefat striped tsol-announcement-audit"><thead><tr><th>' . esc_html__('Time (UTC)', 'tomschooloflife-plugin') . '</th><th>' . esc_html__('Event', 'tomschooloflife-plugin') . '</th><th>' . esc_html__('Actor ID', 'tomschooloflife-plugin') . '</th></tr></thead><tbody>';
    ...
    public function save_post($post_id, $post, $update) {
        ...
        if (TSOL_Library_Announcement_Model::text_length($summary) > TSOL_Library_Announcement_Model::MAX_SUMMARY_LENGTH) {
            ...
            TSOL_Library_Announcement_Model::queue_notice('error', __('The summary was limited to 500 characters.', 'tomschooloflife-plugin'));
        }
    ...
    public function ajax_preview() {
        ...
        if (!TSOL_Library_Announcement_Flags::preview_enabled() || !current_user_can(TSOL_Library_Announcement_Model::CAP_MANAGE_AUDIENCE)) {
            wp_send_json_error(array('message' => __('Audience preview is unavailable.', 'tomschooloflife-plugin')), 403);
        }
        $post_id = isset($_POST['postId']) ? absint($_POST['postId']) : 0;
        if ($post_id <= 0 || TSOL_Library_Announcement_Model::POST_TYPE !== get_post_type($post_id) || !current_user_can('edit_post', $post_id)) {
            wp_send_json_error(array('message' => __('The announcement could not be previewed.', 'tomschooloflife-plugin')), 403);
        }
        if (!$this->consume_rate_limit('preview', 10)) {
            wp_send_json_error(array('message' => __('Please wait before requesting another preview.', 'tomschooloflife-plugin')), 429);
        }
        ...
        if (is_wp_error($definition) || is_wp_error($definition_hash) || $stored_hash === '' || !hash_equals($stored_hash, $definition_hash) || is_wp_error($destination) || !$this->definition_matches_destination($definition, $destination)) {
            ...
            wp_send_json_error(array('message' => __('Save a valid current destination and audience before previewing.', 'tomschooloflife-plugin')), 409);
        }
        ...
        if (is_wp_error($result)) {
            ...
            wp_send_json_error(array('message' => __('The complete audience preview is temporarily unavailable. No partial counts were saved.', 'tomschooloflife-plugin')), 503);
        }
    ...
    public function ajax_user_search() {
        check_ajax_referer(self::AJAX_NONCE_ACTION, 'nonce');
        if (!current_user_can(TSOL_Library_Announcement_Model::CAP_MANAGE_AUDIENCE)) {
            wp_send_json_error(array('message' => __('User search is unavailable.', 'tomschooloflife-plugin')), 403);
        }
        $term = isset($_GET['term']) ? sanitize_text_field(wp_unslash($_GET['term'])) : '';
        if (TSOL_Library_Announcement_Model::text_length($term) < 3 || !$this->consume_rate_limit('user_search', 30)) {
            wp_send_json_error(array('message' => __('Enter at least three characters.', 'tomschooloflife-plugin')), 400);
        }
    ...
    public function render_notices() {
        ...
        echo '<div class="notice notice-info"><p><strong>' . esc_html__('Announcement authoring is in draft-only review.', 'tomschooloflife-plugin') . '</strong> ' . esc_html__('Nothing on this screen can publish, schedule, email, fan out, or change a member’s access.', 'tomschooloflife-plugin') . '</p></div>';
    }

    public function list_columns($columns) {
        $result = array(
            'cb' => $columns['cb'] ?? '<input type="checkbox">',
            'title' => __('Subject', 'tomschooloflife-plugin'),
            'tsol_announcement_state' => __('State', 'tomschooloflife-plugin'),
            'tsol_announcement_destination' => __('Destination', 'tomschooloflife-plugin'),
            'tsol_announcement_audience' => __('Audience', 'tomschooloflife-plugin'),
            'author' => __('Author', 'tomschooloflife-plugin'),
            'date' => __('Updated', 'tomschooloflife-plugin'),
        );
        if (current_user_can(TSOL_Library_Announcement_Model::CAP_VIEW_DELIVERY)) {
            $result = array_slice($result, 0, 5, true) + array(
                'tsol_announcement_schedule' => __('Scheduled / sent', 'tomschooloflife-plugin'),
                'tsol_announcement_recipients' => __('Recipients', 'tomschooloflife-plugin'),
                'tsol_announcement_health' => __('Delivery health', 'tomschooloflife-plugin'),
    ...
    public function render_list_column($column, $post_id) {
        ...
        switch ($column) {
            case 'tsol_announcement_state':
                echo '<span class="tsol-announcement-badge">' . esc_html__('Draft only', 'tomschooloflife-plugin') . '</span>';
                break;
            ...
            case 'tsol_announcement_health':
                echo esc_html__('Not sent', 'tomschooloflife-plugin');
                break;
        }
    }
    ...
    public function updated_messages($messages) {
        $messages[TSOL_Library_Announcement_Model::POST_TYPE] = array_fill(0, 11, __('Announcement draft saved. Nothing was sent.', 'tomschooloflife-plugin'));
        return $messages;
    }
    ...
    private function render_user_picker($field, $label, $ids) {
        ...
        <div class="tsol-announcement-user-picker" data-user-picker data-field="<?php echo esc_attr($field); ?>">
            ...
            <p class="description"><strong data-user-count><?php echo esc_html(count($ids)); ?></strong> <?php esc_html_e('selected; 100 unique users maximum across recipients and exclusions.', 'tomschooloflife-plugin'); ?></p>
            <label class="screen-reader-text" for="tsol-<?php echo esc_attr($field); ?>-search"><?php echo esc_html(sprintf(__('Search for %s', 'tomschooloflife-plugin'), strtolower($label))); ?></label>
            <div class="tsol-announcement-user-search">
                <input id="tsol-<?php echo esc_attr($field); ?>-search" type="search" placeholder="<?php esc_attr_e('Search name or email (3+ characters)', 'tomschooloflife-plugin'); ?>" autocomplete="off" data-user-search>
    ...
    private function save_expiry($post_id, $value) {
        ...
        if (!$date || (is_array($errors) && ($errors['warning_count'] > 0 || $errors['error_count'] > 0)) || $date->getTimestamp() <= time()) {
            TSOL_Library_Announcement_Model::queue_notice('error', __('Choose a valid future expiry in the WordPress site timezone.', 'tomschooloflife-plugin'));
            return;
        }
    ...
    private function render_preview_counts($preview) {
        ...
        <div class="tsol-announcement-preview-summary">
            <p><strong><?php echo esc_html(number_format_i18n((int) ($counts['eligible'] ?? 0))); ?></strong> <?php esc_html_e('linked School accounts currently eligible', 'tomschooloflife-plugin'); ?></p>
            <ul>
                <li><?php echo esc_html(sprintf(__('%s WordPress candidates', 'tomschooloflife-plugin'), number_format_i18n((int) ($counts['wordpressCandidates'] ?? 0)))); ?></li>
                <li><?php echo esc_html(sprintf(__('%s unlinked', 'tomschooloflife-plugin'), number_format_i18n((int) ($counts['unlinked'] ?? 0)))); ?></li>
                <li><?php echo esc_html(sprintf(__('%s excluded', 'tomschooloflife-plugin'), number_format_i18n((int) ($counts['excluded'] ?? 0)))); ?></li>
                <li><?php echo esc_html(sprintf(__('%s relationship preference suppressed', 'tomschooloflife-plugin'), number_format_i18n((int) ($counts['relationshipSuppressed'] ?? 0)))); ?></li>
                <li><?php echo esc_html(sprintf(__('%s eligible administrators', 'tomschooloflife-plugin'), number_format_i18n((int) ($counts['eligibleAdministrators'] ?? 0)))); ?></li>
            </ul>
            <p class="description"><?php esc_html_e('Estimate only. Live access and School relationships will be checked again at any future dispatch.', 'tomschooloflife-plugin'); ?></p>
        </div>
        <?php
    }
    ...
    private function destinations() {
        ...
        $result = array(__('Courses', 'tomschooloflife-plugin') => array(), __('Series', 'tomschooloflife-plugin') => array());
        foreach ($posts as $post) {
            $label = TSOL_Library_Content_Model::COURSE_POST_TYPE === $post->post_type ? __('Courses', 'tomschooloflife-plugin') : __('Series', 'tomschooloflife-plugin');
            $result[$label][] = $post;
        }
        return $result;
    }
    ...
    private function destination_label($post_id) {
        ...
        return is_wp_error($destination) ? __('Unavailable destination', 'tomschooloflife-plugin') : (string) $destination['title'];
    }

    private function preset_authority($preset) {
        if (TSOL_Library_Announcement_Audience_Builder::PRESET_RELATIONSHIP === $preset) {
            return __('School activity + MemberPress access', 'tomschooloflife-plugin');
        }
        if (TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP === $preset || TSOL_Library_Announcement_Audience_Builder::PRESET_CONTENT_ACCESS === $preset) {
            return __('MemberPress / WordPress authority', 'tomschooloflife-plugin');
        }
        return __('WordPress identity', 'tomschooloflife-plugin');
    }
