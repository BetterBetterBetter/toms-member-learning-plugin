...
class MemberLibrary_Account_Security {
    ...
    private static function navigation_item($container_tag, $account_url = null) {
        ...
        return sprintf(
            ...
            esc_attr($classes),
            esc_url($url),
            esc_html__('Security', 'member-library')
        );
    }
        ...
        <div class="mp_wrapper">
            <?php if ('requested' === $status) : ?>
                <div class="mepr_updated" role="status">
                    <?php esc_html_e('Your Library sessions are being signed out on every device. This may take a moment.', 'member-library'); ?>
                </div>
            <?php elseif ('unavailable' === $status) : ?>
                <div class="mepr_error" role="alert">
                    <?php esc_html_e('We could not request the Library sign-out. Please try again.', 'member-library'); ?>
                </div>
            <?php endif; ?>
                    ...
                    <div class="mp-form-row mepr-field-required">
                        <label for="tsol-library-force-logout-confirmation">
                            <input
                                ...
                                required
                            />
                            <?php esc_html_e('I understand that every active Library session, including this browser, will be signed out.', 'member-library'); ?>
                        </label>
                    </div>

                    <div class="mepr_spacer">&nbsp;</div>
                    <button type="submit" class="mepr-submit">
                        <?php esc_html_e('Sign out of the Library on all devices', 'member-library'); ?>
                    </button>
                </form>
            </section>
        </div>
    ...
    public static function handle_forced_logout() {
        ...
        if ('method_not_allowed' === $decision) {
            wp_die(esc_html__('This action requires a POST request.', 'member-library'), '', array('response' => 405));
        }
        ...
        if ('invalid_request' === $decision) {
            wp_die(esc_html__('The security request could not be verified.', 'member-library'), '', array('response' => 403));
        }
        if ('confirmation_required' === $decision) {
            wp_die(esc_html__('Please confirm that every Library session should be signed out.', 'member-library'), '', array('response' => 400));
        }
    ...
    private static function queue_forced_logout($user_id) {
        return MemberLibrary_Auth_Revocation::queue($user_id, 'user.sessions_forced_logout');
    }
