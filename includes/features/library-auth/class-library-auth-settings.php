...
class TSOL_Library_Auth_Settings {
    ...
    public function sanitize_app_url($value) {
        ...
        if ((string) $value !== '' && $url === '') {
            add_settings_error(self::APP_URL_OPTION, 'invalid_url', __('Enter an HTTPS origin with no path, query, fragment, or credentials. HTTP is allowed only for localhost.', 'libertyclassroom-library'));
    ...
    public function sanitize_client_id($value) {
        ...
        if ($value !== '' && !preg_match('/^[A-Za-z0-9._-]{3,128}$/', $value)) {
            add_settings_error(self::CLIENT_ID_OPTION, 'invalid_client_id', __('Use 3–128 letters, numbers, dots, underscores, or hyphens.', 'libertyclassroom-library'));
    ...
    public function sanitize_secret($value) {
        ...
        if (strlen($value) < 32) {
            add_settings_error(self::CLIENT_SECRET_OPTION, 'short_secret', __('The Library client secret must be at least 32 characters.', 'libertyclassroom-library'));
            return (string) get_option(self::CLIENT_SECRET_OPTION, '');
        }
        return $value;
    }
    ...
    public function sanitize_catalogue_webhook_secret($value) {
        ...
        if (strlen($value) < 32) {
            add_settings_error(self::CATALOGUE_WEBHOOK_SECRET_OPTION, 'short_secret', __('The catalogue synchronization secret must be at least 32 characters.', 'libertyclassroom-library'));
            return (string) get_option(self::CATALOGUE_WEBHOOK_SECRET_OPTION, '');
        }
        if (self::client_secret() !== '' && hash_equals(self::client_secret(), $value)) {
            add_settings_error(self::CATALOGUE_WEBHOOK_SECRET_OPTION, 'reused_secret', __('The catalogue synchronization secret must be different from the Library client secret.', 'libertyclassroom-library'));
            return (string) get_option(self::CATALOGUE_WEBHOOK_SECRET_OPTION, '');
        }
        return $value;
    }
    ...
    public function render($embedded = false) {
        ...
        <?php if (!$embedded) : ?><div class="wrap"><?php endif; ?>
            <?php if ($embedded) : ?>
                <h2><?php esc_html_e('Library Authentication', 'libertyclassroom-library'); ?></h2>
            <?php else : ?>
                <h1><?php esc_html_e('Library Authentication', 'libertyclassroom-library'); ?></h1>
            <?php endif; ?>
            <p><?php esc_html_e('WordPress authenticates every Library user. MemberPress rules authorize each protected course or lesson, while WordPress administrator permissions continue to authorize admins.', 'libertyclassroom-library'); ?></p>
            <?php settings_errors(); ?>
            <?php if ($reason === '') : ?>
                <div class="notice notice-success inline"><p><?php esc_html_e('The Library authentication bridge is ready.', 'libertyclassroom-library'); ?></p></div>
            <?php else : ?>
                <div class="notice notice-warning inline"><p><?php echo esc_html($reason); ?></p></div>
            <?php endif; ?>
            <?php if ($reason === '' && !$memberpress_ready) : ?>
                <div class="notice notice-warning inline"><p><?php esc_html_e('WordPress sign-in is ready, but MemberPress is unavailable. Users can authenticate, while protected content will remain unavailable until MemberPress returns.', 'libertyclassroom-library'); ?></p></div>
            <?php endif; ?>
            <?php if (!$secret_from_constant) : ?>
                <div class="notice notice-info inline"><p><?php esc_html_e('This site stores the Library client secret as a write-only WordPress setting because no server-managed TSOL_LIBRARY_CLIENT_SECRET constant is available. The value is never displayed after saving. A server-managed constant remains the preferred option when hosting access becomes available.', 'libertyclassroom-library'); ?></p></div>
            <?php endif; ?>
            <form method="post" action="options.php">
                <?php settings_fields('tsol_library_auth'); ?>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><?php esc_html_e('Enable bridge', 'libertyclassroom-library'); ?></th>
                        <td><label><input type="checkbox" name="<?php echo esc_attr(self::ENABLED_OPTION); ?>" value="1" <?php checked(self::enabled()); ?>> <?php esc_html_e('Allow authenticated WordPress users to sign in to the Library', 'libertyclassroom-library'); ?></label></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="tsol-library-app-url"><?php esc_html_e('Library URL', 'libertyclassroom-library'); ?></label></th>
                        <td><input id="tsol-library-app-url" class="regular-text code" type="url" name="<?php echo esc_attr(self::APP_URL_OPTION); ?>" value="<?php echo esc_attr((string) get_option(self::APP_URL_OPTION, '')); ?>" placeholder="http://localhost:3000"><p class="description"><?php esc_html_e('Only the exact Better Auth callback on this origin is accepted. TSOL_LIBRARY_APP_URL overrides this setting.', 'libertyclassroom-library'); ?></p></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="tsol-library-client-id"><?php esc_html_e('Client ID', 'libertyclassroom-library'); ?></label></th>
                        <td><input id="tsol-library-client-id" class="regular-text code" type="text" name="<?php echo esc_attr(self::CLIENT_ID_OPTION); ?>" value="<?php echo esc_attr((string) get_option(self::CLIENT_ID_OPTION, 'liberty-classroom-library')); ?>"><p class="description"><?php esc_html_e('Use a different client ID per environment. TSOL_LIBRARY_CLIENT_ID overrides this setting.', 'libertyclassroom-library'); ?></p></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="tsol-library-client-secret"><?php esc_html_e('Client secret', 'libertyclassroom-library'); ?></label></th>
                        <td>
                            <p style="margin: 0 0 10px;">
                                <?php if ($secret_is_valid) : ?>
                                    <span data-library-secret-status="configured" style="align-items: center; background: #edfaef; border: 1px solid #8fd19e; border-radius: 999px; color: #166534; display: inline-flex; font-weight: 600; gap: 5px; padding: 4px 10px;">
                                        ...
                                        <?php esc_html_e('Configured', 'libertyclassroom-library'); ?>
                                    </span>
                                <?php elseif ($secret_is_present) : ?>
                                    <span data-library-secret-status="invalid" style="align-items: center; background: #fcf0f1; border: 1px solid #d63638; border-radius: 999px; color: #8a2424; display: inline-flex; font-weight: 600; gap: 5px; padding: 4px 10px;">
                                        ...
                                        <?php esc_html_e('Needs replacement', 'libertyclassroom-library'); ?>
                                    </span>
                                <?php else : ?>
                                    <span data-library-secret-status="missing" style="align-items: center; background: #f6f7f7; border: 1px solid #c3c4c7; border-radius: 999px; color: #50575e; display: inline-flex; font-weight: 600; gap: 5px; padding: 4px 10px;">
                                        ...
                                        <?php esc_html_e('Not configured', 'libertyclassroom-library'); ?>
                                    </span>
                                <?php endif; ?>
                                <span class="description" style="margin-left: 6px;">
                                    <?php echo esc_html($secret_from_constant ? __('Provided by the server environment', 'libertyclassroom-library') : ($secret_is_present ? __('Saved in WordPress; the value is never displayed', 'libertyclassroom-library') : __('No secret has been provided yet', 'libertyclassroom-library'))); ?>
                                </span>
                            </p>
                            <input id="tsol-library-client-secret" class="regular-text code" type="password" autocomplete="new-password" name="<?php echo esc_attr(self::CLIENT_SECRET_OPTION); ?>" value="" <?php disabled($secret_from_constant); ?> placeholder="<?php echo esc_attr($secret_is_present ? __('Enter a new secret to replace the current one', 'libertyclassroom-library') : __('At least 32 characters', 'libertyclassroom-library')); ?>">
                            <p class="description"><?php echo esc_html($secret_from_constant ? __('This field is disabled because the server environment controls the secret.', 'libertyclassroom-library') : __('Leave this field blank to keep the current secret. Entering a value replaces it after you save; the saved value is never displayed.', 'libertyclassroom-library')); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="tsol-library-catalogue-webhook-secret"><?php esc_html_e('Catalogue synchronization secret', 'libertyclassroom-library'); ?></label></th>
                        <td>
                            <p style="margin: 0 0 10px;">
                                ...
                                <?php endif; ?>
                                <span class="description" style="margin-left: 6px;">
                                    <?php echo esc_html($catalogue_secret_from_constant ? __('Provided by the server environment', 'libertyclassroom-library') : ($catalogue_secret_is_present ? __('Saved in WordPress; the value is never displayed', 'libertyclassroom-library') : __('No secret has been provided yet', 'libertyclassroom-library'))); ?>
                                </span>
                            </p>
                            <input id="tsol-library-catalogue-webhook-secret" class="regular-text code" type="password" autocomplete="new-password" name="<?php echo esc_attr(self::CATALOGUE_WEBHOOK_SECRET_OPTION); ?>" value="" <?php disabled($catalogue_secret_from_constant); ?> placeholder="<?php echo esc_attr($catalogue_secret_is_present ? __('Enter a new secret to replace the current one', 'libertyclassroom-library') : __('At least 32 characters', 'libertyclassroom-library')); ?>">
                            <p class="description"><?php echo esc_html($catalogue_secret_from_constant ? __('This field is disabled because the server environment controls the secret.', 'libertyclassroom-library') : __('Paste the exact TSOL_LIBRARY_CATALOGUE_WEBHOOK_SECRET from the School app. Leave blank to keep the current value; the saved value is never displayed.', 'libertyclassroom-library')); ?></p>
                        </td>
                    </tr>
                </table>
                <?php submit_button(); ?>
    ...
    public static function client_id() {
        $value = defined('TSOL_LIBRARY_CLIENT_ID') ? TSOL_LIBRARY_CLIENT_ID : get_option(self::CLIENT_ID_OPTION, 'liberty-classroom-library');
    ...
    public static function readiness_error() {
        if (!self::enabled()) {
            return __('The Library authentication bridge is disabled.', 'libertyclassroom-library');
        }
        if (self::app_url() === '') {
            return __('A valid Library URL is required.', 'libertyclassroom-library');
        }
        if (!preg_match('/^[A-Za-z0-9._-]{3,128}$/', self::client_id())) {
            return __('A valid environment-specific client ID is required.', 'libertyclassroom-library');
        }
        if (strlen(self::client_secret()) < 32) {
            return __('A client secret of at least 32 characters is required.', 'libertyclassroom-library');
        }
        return '';
    }
