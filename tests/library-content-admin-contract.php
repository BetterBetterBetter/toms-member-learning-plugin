<?php
/**
 * Contract for the Liberty Classroom Library wp-admin editor.
 *
...
try {
    ...
    if (class_exists('LeadpagesWP\\Admin\\TinyMCE\\LeadboxTinyMCE')) {
        ...
        $editor->isolate_private_editor_integrations();
        foreach ($leadbox_callbacks as $hook_name => $callback) {
            $assert(false === has_filter($hook_name, $callback), 'A LeadPages editor callback leaked onto the private Liberty Classroom Library editor.');
        }
    }
    ...
    $fixture_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::ITEM_POST_TYPE,
        'post_status' => 'draft',
        'post_title' => 'Liberty Classroom Library editor contract fixture',
        'post_content' => '',
    ), true);
    ...
    $speaker_fixture_id = wp_insert_post(array(
        ...
        'post_status' => 'draft',
        'post_title' => 'Zulu Liberty Classroom Library speaker relationship contract fixture',
    ), true);
    ...
    $second_speaker_fixture_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::SPEAKER_POST_TYPE,
        'post_status' => 'publish',
        'post_title' => 'Alpha Liberty Classroom Library speaker relationship contract fixture',
    ), true);
    ...
    $course_fixture_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::COURSE_POST_TYPE,
        'post_status' => 'draft',
        'post_title' => 'Liberty Classroom Library course-column contract fixture',
        'post_content' => '',
    ), true);
    ...
    $assert(false !== strpos($inherited_picker_html, 'Liberty Classroom Library course-column contract fixture'), 'The inherited Speaker panel omitted the parent Course link.');
    $assert(false !== strpos($inherited_picker_html, 'Zulu Liberty Classroom Library speaker relationship contract fixture'), 'The inherited Speaker panel omitted an effective Speaker card.');

    ob_start();
    $editor->render_speakers_column(TSOL_Library_Content_Admin::SPEAKERS_COLUMN, $fixture_id);
    $inherited_column_html = ob_get_clean();
    $assert(false !== strpos($inherited_column_html, 'Alpha Liberty Classroom Library speaker relationship contract fixture'), 'The Content list omitted an inherited Speaker.');
    ...
    $collection_fixture = wp_insert_term(
        'Liberty Classroom Library course-collection contract fixture',
    ...
    $assert(false !== strpos($lesson_column, 'Liberty Classroom Library course-column contract fixture'), 'A course lesson did not show its Course title.');
    ...
    $series_fixture_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::SERIES_POST_TYPE,
        'post_status' => 'draft',
        'post_title' => 'Liberty Classroom Library series-count contract fixture',
    ), true);
    ...
    $assert(!$has_meta_box('mpcs-course', 'tsol-library-course-page-content'), 'Library Course landing-page fields leaked onto a native MemberPress Course.');
    ...
    $assert(false === strpos($details_html, 'Include in the Liberty Classroom Library'), 'The retired editable Library inclusion gate remains in Library details.');
    ...
    $invalid_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::ITEM_POST_TYPE,
        'post_status' => 'publish',
        'post_title' => 'Liberty Classroom Library invalid publish contract fixture',
        'post_content' => '',
    ), true);
    ...
    $rule = new MeprRule();
    $rule->post_title = 'Liberty Classroom Library availability contract rule';
    ...
    $coming_soon_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::ITEM_POST_TYPE,
        'post_status' => 'publish',
        'post_title' => 'Liberty Classroom Library coming-soon contract fixture',
        'post_content' => '',
    ), true);
    ...
    $unprotected_course_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Content_Model::COURSE_POST_TYPE,
        'post_status' => 'publish',
        'post_title' => 'Liberty Classroom Library unprotected publish contract fixture',
    ), true);
...
WP_CLI::success('Liberty Classroom Library content admin contract passed; disposable fixtures were removed.');
