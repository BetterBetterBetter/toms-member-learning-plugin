...
$admin_ui = new TSOL_Library_Announcement_Admin();
$admin_ui->init();

try {
    $post_type = get_post_type_object(TSOL_Library_Announcement_Model::POST_TYPE);
    ...
    $assert($editor_role instanceof WP_Role && $editor_role->has_cap(TSOL_Library_Announcement_Model::CAP_EDIT), 'Editors cannot create announcement drafts.');
    foreach (array(
        TSOL_Library_Announcement_Model::CAP_PUBLISH,
        TSOL_Library_Announcement_Model::CAP_SCHEDULE,
        TSOL_Library_Announcement_Model::CAP_MANAGE_AUDIENCE,
        TSOL_Library_Announcement_Model::CAP_VIEW_DELIVERY,
    ) as $capability) {
    ...
    $matrix_presets = array(
        TSOL_Library_Announcement_Audience_Builder::PRESET_ALL_LINKED => 'AUTHENTICATED_SCHOOL_USER',
        TSOL_Library_Announcement_Audience_Builder::PRESET_CONTENT_ACCESS => 'CAN_ACCESS_CONTENT',
        TSOL_Library_Announcement_Audience_Builder::PRESET_RELATIONSHIP => 'ACTIVE_RELATIONSHIP',
        TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP => 'ACTIVE_MEMBERSHIP',
        TSOL_Library_Announcement_Audience_Builder::PRESET_SPECIFIC_USERS => 'SPECIFIC_USERS',
    );
    foreach ($matrix_destinations as $destination_type => $destination_id) {
        foreach ($matrix_presets as $matrix_preset => $expected_condition) {
            ...
            $matrix_result = TSOL_Library_Announcement_Audience_Builder::build($payload);
            $requires_destination = in_array($matrix_preset, array(
                TSOL_Library_Announcement_Audience_Builder::PRESET_CONTENT_ACCESS,
                TSOL_Library_Announcement_Audience_Builder::PRESET_RELATIONSHIP,
            ), true);
            ...
            if ('general' !== $destination_type) {
                $matrix_destination = TSOL_Library_Announcement_Audience_Builder::destination($destination_id);
            ...
            $assert(array((int) $candidate_user_ids[1]) === TSOL_Library_Announcement_Audience_Builder::exclusion_ids($matrix_result['definition']), sprintf('The %s + %s combination lost its exclusion.', $destination_type, $matrix_preset));
        }
    }

    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP,
        'destination_id' => 0,
    ...
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP,
        'destination_id' => 0,
        'membership_ids' => range(1, TSOL_Library_Announcement_Audience_Contract::MAX_MEMBERSHIPS + 1),
    ...
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_SPECIFIC_USERS,
        'destination_id' => 0,
    ...
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_SPECIFIC_USERS,
        'destination_id' => 0,
        'specific_user_ids' => range(1, TSOL_Library_Announcement_Audience_Contract::MAX_SPECIFIC_USERS + 1),
    ...
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_ALL_LINKED,
        'destination_id' => 0,
        'exclude_user_ids' => range(1, TSOL_Library_Announcement_Audience_Contract::MAX_SPECIFIC_USERS + 1),
    ))), 'The audience accepted more than 100 excluded users.');

    $destination = TSOL_Library_Announcement_Audience_Builder::destination($destinations[0]->ID);
    $assert(!is_wp_error($destination), 'A published TSOL destination was rejected.');
    $all_linked = TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_ALL_LINKED,
    ...
    $relationship = TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_RELATIONSHIP,
        'destination_id' => $destinations[0]->ID,
    ));
    ...
    $membership = TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_MEMBERSHIP,
        'destination_id' => 0,
    ...
    if (!is_wp_error($membership)) {
        $ids = TSOL_Library_Announcement_Audience_Builder::ids_for_condition($membership['definition'], 'ACTIVE_MEMBERSHIP', 'membershipIds');
    ...
    $specific = TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_SPECIFIC_USERS,
        'destination_id' => 0,
    ...
    if (!is_wp_error($specific)) {
        $ids = TSOL_Library_Announcement_Audience_Builder::ids_for_condition($specific['definition'], 'SPECIFIC_USERS', 'wordpressUserIds');
        ...
        $assert(array((int) $candidate_user_ids[1]) === TSOL_Library_Announcement_Audience_Builder::exclusion_ids($specific['definition']), 'Specific-user exclusions were not retained.');
    ...
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::build(array(
        'audience_preset' => TSOL_Library_Announcement_Audience_Builder::PRESET_CONTENT_ACCESS,
        'destination_id' => 0,
    ))), 'The content-access preset accepted a missing destination.');
    $assert(is_wp_error(TSOL_Library_Announcement_Audience_Builder::destination(PHP_INT_MAX)), 'A missing destination was accepted.');

    $sanitized = TSOL_Library_Announcement_Model::sanitize_body('<script>private()</script><h1>Heading</h1><p>Hello <a href="https://example.test">link</a></p><ul><li>Item</li></ul>');
    ...
    $announcement_id = wp_insert_post(array(
        'post_type' => TSOL_Library_Announcement_Model::POST_TYPE,
        'post_status' => 'draft',
    ...
    $long_title = str_repeat('x', TSOL_Library_Announcement_Model::MAX_SUBJECT_LENGTH + 10);
    $filtered = TSOL_Library_Announcement_Model::filter_post_data(array(
        'post_type' => TSOL_Library_Announcement_Model::POST_TYPE,
        'post_status' => 'publish',
        'post_title' => $long_title,
        'post_content' => str_repeat('y', TSOL_Library_Announcement_Model::MAX_BODY_LENGTH + 1),
    ), array('ID' => $announcement_id));
    $assert('draft' === $filtered['post_status'], 'A forged publish status bypassed the draft-only flag.');
    $assert(TSOL_Library_Announcement_Model::MAX_SUBJECT_LENGTH === TSOL_Library_Announcement_Model::text_length(wp_unslash($filtered['post_title'])), 'The subject length limit is not enforced.');
    ...
    $previous_post = $_POST;
    $_POST = array(
        TSOL_Library_Announcement_Admin::NONCE_NAME => wp_create_nonce(TSOL_Library_Announcement_Admin::NONCE_ACTION),
        TSOL_Library_Announcement_Admin::PAYLOAD_NAME => array(
    ...
    $_POST = $previous_post;
    $saved_hash = (string) get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_AUDIENCE_HASH, true);
    $assert((bool) preg_match('/^[a-f0-9]{64}$/', $saved_hash), 'The guided editor did not persist a normalized audience hash.');
    $assert('A safe synthetic summary.' === get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_SUMMARY, true), 'The bounded summary was not saved.');
    $assert($destination['uuid'] === get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_DESTINATION_UUID, true), 'The canonical destination UUID was not saved.');
    $administrator_expiry = gmdate('Y-m-d H:i:s', time() + DAY_IN_SECONDS);
    update_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_EXPIRY_GMT, $administrator_expiry);
    ...
    $admin_ui->save_post($announcement_id, get_post($announcement_id), true);
    $_POST = $previous_post;
    $assert($saved_hash === get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_AUDIENCE_HASH, true), 'An editor without the audience capability changed the audience.');
    $assert('Editor-updated copy.' === get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_SUMMARY, true), 'An editor could not revise permitted draft copy.');
    $assert($administrator_expiry === get_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_EXPIRY_GMT, true), 'An editor without delivery authority changed the visibility expiry.');
    ...
    $assert(!is_wp_error(TSOL_Library_Announcement_Preview::validate_result($preview, $saved_hash)), 'A valid aggregate preview was rejected because its JSON key order changed.');
    update_post_meta($announcement_id, TSOL_Library_Announcement_Model::META_PREVIEW, $preview);
    $preview['unexpected'] = true;
    $assert(is_wp_error(TSOL_Library_Announcement_Preview::validate_result($preview, $saved_hash)), 'An aggregate preview with an unknown field was accepted.');
    unset($preview['unexpected']);
    $assert(is_wp_error(TSOL_Library_Announcement_Preview::validate_result($preview, str_repeat('0', 64))), 'An aggregate preview with the wrong audience hash was accepted.');
    ...
    for ($index = 0; $index < TSOL_Library_Announcement_Audit::MAX_ENTRIES + 5; $index++) {
        TSOL_Library_Announcement_Audit::record($announcement_id, 'draft_updated', array(
            'definitionHash' => $saved_hash,
            'email' => 'must-not-be-stored@example.test',
        ));
    }
    $audit = TSOL_Library_Announcement_Audit::entries($announcement_id);
    $assert(TSOL_Library_Announcement_Audit::MAX_ENTRIES === count($audit), 'The editorial audit exceeded its bounded retention.');
    ...
    $revision_keys = TSOL_Library_Announcement_Model::revision_meta_keys(array());
    $assert(in_array(TSOL_Library_Announcement_Model::META_AUDIENCE, $revision_keys, true), 'Audience metadata is not revisioned.');
    $assert(in_array(TSOL_Library_Announcement_Model::META_DESTINATION_UUID, $revision_keys, true), 'Destination metadata is not revisioned.');
    ...
    $assert(false === TSOL_Library_Announcement_Flags::publish_enabled(), 'The publish/schedule flag is not fail-closed.');
    $assert(false === TSOL_Library_Announcement_Flags::self_test_enabled(), 'The self-test delivery flag is not fail-closed.');
} finally {
