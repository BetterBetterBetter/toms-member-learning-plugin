...
try {
    ...
    $term = wp_insert_term('Collection editorial contract', TSOL_Library_Content_Model::COURSE_COLLECTION_TAXONOMY, array(
    ...
    update_post_meta($created_course_id, TSOL_Library_Content_Model::META_UUID, wp_generate_uuid4());
    update_post_meta($created_course_id, TSOL_Library_Content_Model::META_AUTHORIZATION_POST_ID, $created_course_id);
    update_post_meta($created_course_id, TSOL_Library_Content_Model::META_MIGRATION_KEY, 'collection-editorial-course');
    update_post_meta($created_course_id, TSOL_Library_Content_Model::META_MIGRATION_VERSION, '1');
    update_post_meta($created_course_id, TSOL_Library_Content_Model::META_CONTENT_FINGERPRINT, 'collection-editorial-fingerprint');
    wp_set_object_terms($created_course_id, array($created_term_id), TSOL_Library_Content_Model::COURSE_COLLECTION_TAXONOMY);

    $admin = new TSOL_Library_Collection_Admin();
    $_POST = array(
        TSOL_Library_Collection_Admin::NONCE_NAME => wp_create_nonce(TSOL_Library_Collection_Admin::NONCE_ACTION),
        TSOL_Library_Collection_Admin::PAYLOAD_NAME => array(
    ...
    $stored_overview = (string) get_term_meta($created_term_id, TSOL_Library_Content_Model::COLLECTION_META_OVERVIEW, true);
    ...
    ob_start();
    $admin->render_edit_fields(get_term($created_term_id), TSOL_Library_Content_Model::COURSE_COLLECTION_TAXONOMY);
