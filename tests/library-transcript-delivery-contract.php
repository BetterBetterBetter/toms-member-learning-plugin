...
$assert(class_exists('MemberLibrary_Content_Transcripts'), 'Transcript delivery is not loaded.');
$assert(false !== has_action(MemberLibrary_Content_Transcripts::RETRY_HOOK, array('MemberLibrary_Content_Transcripts', 'deliver')), 'Transcript retry handler is not registered.');
$assert(false === has_action('add_meta_boxes_' . MemberLibrary_Content_Model::ITEM_POST_TYPE, array('MemberLibrary_Content_Transcripts', 'add_meta_box')), 'Transcript upload still registers a separate metabox.');

$transcript_keys = array(
    MemberLibrary_Content_Model::META_TRANSCRIPT_CONTENT,
    MemberLibrary_Content_Model::META_TRANSCRIPT_HASH,
    MemberLibrary_Content_Model::META_TRANSCRIPT_LANGUAGE,
    MemberLibrary_Content_Model::META_TRANSCRIPT_FILENAME,
    MemberLibrary_Content_Model::META_TRANSCRIPT_MODIFIED_AT,
    MemberLibrary_Content_Model::META_TRANSCRIPT_VERSION,
);
$item_keys = MemberLibrary_Content_Model::metadata_keys_for_post_type(MemberLibrary_Content_Model::ITEM_POST_TYPE);
...
foreach (array(MemberLibrary_Content_Model::COURSE_POST_TYPE, MemberLibrary_Content_Model::SERIES_POST_TYPE) as $post_type) {
    foreach ($transcript_keys as $key) {
        $assert(!in_array($key, MemberLibrary_Content_Model::metadata_keys_for_post_type($post_type), true), sprintf('Transcript source key %s leaked onto %s.', $key, $post_type));
    }
}

$vtt = "WEBVTT\n\n00:00:00.000 --> 00:00:02.000\nContract transcript.\n";
$assert($vtt === MemberLibrary_Content_Model::sanitize_transcript_vtt($vtt), 'A valid WebVTT source was changed.');
$assert('' === MemberLibrary_Content_Model::sanitize_transcript_vtt('plain text'), 'A non-WebVTT source was accepted.');
...
update_post_meta($post_id, MemberLibrary_Content_Model::META_UUID, $uuid);
update_post_meta($post_id, MemberLibrary_Content_Model::META_MEDIA_ASSETS, array(array(
    'key' => 'primary',
    'provider' => 'vimeo',
...
update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_FILENAME, 'lesson.vtt');
update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_MODIFIED_AT, $modified_at);
update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_VERSION, 3);
update_post_meta($post_id, MemberLibrary_Content_Model::META_TRANSCRIPT_CONTENT, wp_slash($vtt));

$issued_at = 1787832000;
$payload = MemberLibrary_Content_Transcripts::delivery_payload($post_id, $issued_at);
...
$assert(true === MemberLibrary_Content_Transcripts::deliver($post_id), 'A valid transcript delivery was not accepted.');
...
if (!empty($requests)) {
    ...
    $assert(MemberLibrary_Content_Transcripts::ENDPOINT_PATH === wp_parse_url($request['url'], PHP_URL_PATH), 'Transcript delivery used the wrong endpoint.');
...
$assert('delivered' === get_post_meta($post_id, MemberLibrary_Content_Transcripts::STATUS_META, true), 'Successful delivery was not recorded.');
...
$scheduled = wp_next_scheduled(MemberLibrary_Content_Transcripts::RETRY_HOOK, array((int) $post_id));
if (false !== $scheduled) {
    wp_unschedule_event($scheduled, MemberLibrary_Content_Transcripts::RETRY_HOOK, array((int) $post_id));
}
