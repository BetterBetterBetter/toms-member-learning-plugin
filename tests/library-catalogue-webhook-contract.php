...
$assert(has_action(TSOL_Library_Catalogue_Webhook::WATCHDOG_HOOK, array('TSOL_Library_Catalogue_Webhook', 'deliver_pending')) !== false, 'The recurring webhook recovery watchdog is not registered.');
...
$existing_watchdog = wp_next_scheduled(TSOL_Library_Catalogue_Webhook::WATCHDOG_HOOK);
$missing_option_marker = new stdClass();
$existing_last_delivery = get_option(TSOL_Library_Catalogue_Webhook::LAST_DELIVERY_OPTION, $missing_option_marker);
$assert(0 === $existing_rows, 'The webhook outbox must be drained before running the isolated delivery contract.');
$assert(false !== wp_next_scheduled(TSOL_Library_Catalogue_Webhook::WATCHDOG_HOOK), 'The one-minute delivery watchdog was not installed.');
...
if (0 === $existing_rows) {
    ...
    TSOL_Library_Catalogue_Webhook::queue_change($cursor + 1);
    TSOL_Library_Catalogue_Webhook::flush_queued_change();
...
if (false === $existing_watchdog) {
    wp_clear_scheduled_hook(TSOL_Library_Catalogue_Webhook::WATCHDOG_HOOK);
}
...
if ($missing_option_marker === $existing_last_delivery) {
    delete_option(TSOL_Library_Catalogue_Webhook::LAST_DELIVERY_OPTION);
} else {
    update_option(TSOL_Library_Catalogue_Webhook::LAST_DELIVERY_OPTION, $existing_last_delivery, false);
}
