...
2. The plugin checks `https://github.com/BetterBetterBetter/tomschooloflife-plugin` for newer GitHub releases.
...
2. Constant: `define('TSOL_SITE_PLUGIN_VERSION', '0.1.1');`
...
zip -r tomschooloflife-plugin-0.1.1.zip tomschooloflife-plugin \
  -x "tomschooloflife-plugin/.git/*" \
  -x "tomschooloflife-plugin/.DS_Store" \
  -x "tomschooloflife-plugin/*.zip"
```

The ZIP should contain a top-level `tomschooloflife-plugin/` folder with `tomschooloflife-plugin.php` inside it.
...
4. Add release notes.
5. Upload `tomschooloflife-plugin-0.1.1.zip` as a release asset.
6. Publish the release.

Option B: GitHub CLI

```bash
gh release create v0.1.1 tomschooloflife-plugin-0.1.1.zip \
